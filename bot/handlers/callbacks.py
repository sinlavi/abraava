...
async def handle_callback(bot, callback_query, api_client, user_settings_service,
    ...
    # Compatibility layer for Bale (CallbackQuery) and Telethon (CallbackQuery.Event)
    data = getattr(callback_query, 'data', None)
    if isinstance(data, bytes): data = data.decode()

    # Telethon data can be None if not provided in Button.inline
    if data is None: return

    parts = data.split(":")

    message = getattr(callback_query, 'message', None)
    if not message: return # Should not happen

    chat_id = message.chat.id

    user_id = getattr(callback_query.author, 'id', getattr(callback_query, 'sender_id', None))
    callback_id = str(getattr(callback_query, 'id', ''))
    ...
    if owner_id and user_id != owner_id:
        await bot.answer_callback_query(callback_id, text="⚠️ شما دسترسی به این پیام را ندارید. این پیام برای کاربر دیگری ساخته شده است.", show_alert=True)
        return
    ...
    if data == "ignore":
        await bot.answer_callback_query(callback_id, text="")
        return

    if data.startswith("copy:"):
        text = data[5:]
        await bot.answer_callback_query(callback_id, text=f"✅ پیوند کپی شد:\n{text}", show_alert=True)
        return
    ...
    if data.startswith("confirm_dl:"):
        ...
        if url:
            ...
            await bot.answer_callback_query(callback_id, text="⬇️ در حال دانلود...")
            ...
            await bot.answer_callback_query(callback_id, text="❌ پیوند منقضی شده است", show_alert=True)
        return
    ...
    if data.startswith("confirm_") and ":" in data:
        ...
        if setting_type in ["quick_mode", "show_artwork", "auto_download", "notifications"]:
            ...
            await bot.answer_callback_query(callback_id, text="✅ تنظیمات ذخیره شد")
    ...
    if data.startswith("set_quality:"):
        ...
        await bot.answer_callback_query(callback_id, text=f"✅ کیفیت به {q} تغییر یافت")
        await update_settings_msg(bot, callback_query.message, user_id, user_settings_service)
        return
    ...
    if data.startswith("artist:"):
        ...
        if tracks_data and tracks_data.get("results"):
            ...
            else: await bot.answer_callback_query(callback_id, text="❌ خطایی رخ داد", show_alert=True)
    ...
    # Searches
        ...
        if cached:
            ...
            await bot.answer_callback_query(callback_id, text="جستجو منقضی شده است", show_alert=True)
    ...
    # Downloads
        ...
        if settings.download_quality == DownloadQuality.ASK:
            ...
            await bot.answer_callback_query(callback_id, text="⏳ در حال آماده‌سازی...")
        ...
        if track_id.isdigit(): track_id = int(track_id)
        await bot.answer_callback_query(callback_id, text=f"⏳ دانلود با کیفیت {quality}...")
        ...
        track_id = parts[1]
        if track_id.isdigit(): track_id = int(track_id)
        await bot.answer_callback_query(callback_id, text="⏳ در حال دریافت...")
        ...
        if settings.download_quality == DownloadQuality.ASK:
            ...
            await bot.answer_callback_query(callback_id, text="📀 شروع دانلود آلبوم...")
        ...
        if coll_id.isdigit(): coll_id = int(coll_id)
        await bot.answer_callback_query(callback_id, text=f"📀 شروع دانلود با کیفیت {quality}...")
        ...
        await bot.answer_callback_query(callback_id, text="🔄 تلاش مجدد برای قطعات ناموفق...")
        settings = await user_settings_service.get_settings(user_id)
        ...
        await bot.answer_callback_query(callback_id, text="⏹️ توقف دانلود...")

    elif data.startswith("force_artwork:"):
        # Logic to force download/upload artwork
        await bot.answer_callback_query(callback_id, text="⏳ تلاش مجدد با دانلود مستقیم...")
