from core.config import BOT_NAME, INFO_CHANNEL_ID, REQUIRED_CHANNELS, PLATFORM
...
async def start_command(bot, message):
    ...
    if REQUIRED_CHANNELS:
        for channel in REQUIRED_CHANNELS:
            url = f"https://t.me/{channel['username'].lstrip('@')}" if PLATFORM == "telegram" else f"https://ble.ir/{channel['username'].lstrip('@')}"
            markup.append([{"text": f"📢 عضویت در {channel['name']}", "url": url}])
    await send_message(bot, message.chat_id, welcome_text, reply_markup=markup if markup else None)
...
async def help_command(bot, message, is_callback=False):
    ...
    if is_callback: await edit_message(message, help_text)
    else: await send_message(bot, message.chat_id, help_text)
