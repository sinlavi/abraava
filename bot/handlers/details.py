...
async def show_artist_page(bot, chat_id, artist_id, page, artwork_service, owner_id, message_to_edit=None, force=False, is_pagination=False, reply_to=None):
    ...
    try:
            ...
            for i, coll in enumerate(page_items, (page-1)*ITEMS_PER_PAGE + 1):
                if coll.get('wrapperType') in ['collection', 'track']:
                    ...
                    if is_single:
                        ...
                        markup_rows.append([Button(text=btn_text, callback_data=f"single_album:{item_id}:u{owner_id}")])
                    else:
                        btn_text = f"\u200e{(i - 1)}. {name[:35]}{year_str} 📀"
                        markup_rows.append([Button(text=btn_text, callback_data=f"collection:{coll['collectionId']}:u{owner_id}")])
        ...
        markup_rows.append([
            Button(text="🔄 تازه‌سازی", callback_data=f"recrawl:artist:{artist_id}:u{owner_id}"),
            Button(text="📋 کپی پیوند", copy_text=f"{DEEP_LINK_BASE}artist_{artist_id}")
        ])
        markup_rows.append([
            Button(text="🌐 اطلاعات بیشتر", url=itunes_url),
            Button(text="🔍 جستجوی آهنگ‌ها", callback_data=f"refine:track:{artist_name}:u{owner_id}")
        ])
...
async def show_collection_page(bot, chat_id, collection_id, page, artwork_service, owner_id, message_to_edit=None, force=False, is_pagination=False, reply_to=None):
    ...
    try:
        ...
        if tracks:
            ...
            is_private = getattr(chat, 'type', '') not in ["group", "supergroup"]
            if is_private:
                markup_rows.append([Button(text="⬇️ دانلود کل آلبوم", callback_data=f"download_album:{collection_id}:u{owner_id}")])

        if artist_id:
            markup_rows.append([Button(text="🎤 مشاهده هنرمند", callback_data=f"artist:{artist_id}:u{owner_id}")])

        itunes_url = coll.get('collectionViewUrl') or coll.get('viewUrl') or f"https://music.apple.com/album/{collection_id}"
        markup_rows.append([
            Button(text="🔄 تازه‌سازی", callback_data=f"recrawl:collection:{collection_id}:u{owner_id}"),
            Button(text="📋 کپی پیوند", copy_text=f"{DEEP_LINK_BASE}collection_{collection_id}")
        ])
        markup_rows.append([
            Button(text="🌐 اطلاعات بیشتر", url=itunes_url),
            Button(text="🎤 آثار دیگر هنرمند", callback_data=f"artist:{artist_id}:u{owner_id}")
        ])
        ...
        retry_markup = [[Button(text="🔄 تلاش مجدد", callback_data=f"collection:{collection_id}:1:u{owner_id}")]]
...
async def show_track_page(bot, chat_id, track_id, artwork_service, owner_id, message_to_edit=None, reply_to=None):
    ...
    try:
        ...
        markup_rows = []
        dl_btns = [Button(text="⬇️ دانلود", callback_data=f"download:{track_id}:u{owner_id}")]
        if not is_sc and track.get("previewUrl"):
            dl_btns.append(Button(text="🎧 پیش‌نمایش", callback_data=f"preview:{track_id}:u{owner_id}"))
        markup_rows.append(dl_btns)

        # Lyrics button
        if not is_sc:
            markup_rows.append([Button(text="📜 متن آهنگ", callback_data=f"lyrics:{track_id}:u{owner_id}")])

        links = []
        if collection_id: links.append(Button(text="📀 مشاهده آلبوم", callback_data=f"collection:{collection_id}:u{owner_id}"))
        if artist_id: links.append(Button(text="🎤 مشاهده هنرمند", callback_data=f"artist:{artist_id}:u{owner_id}"))
        ...
        markup_rows.append([
            Button(text="📋 کپی پیوند", copy_text=f"{DEEP_LINK_BASE}track_{track_id}"),
            Button(text="🌐 اطلاعات بیشتر", url=itunes_url)
        ])
