from core.config import INFO_CHANNEL_USERNAME, PLATFORM

def create_close_button(user_id: int = None):
    cb = f"close:u{user_id}" if user_id else "close"
    return {"text": "❌ بستن", "callback_data": cb}

def create_cancel_button(task_id: str, user_id: int = None):
    cb = f"cancel_task:{task_id}"
    if user_id: cb += f":u{user_id}"
    return {"text": "⏹️ توقف", "callback_data": cb}

def create_info_channel_button():
    url = f"https://t.me/{INFO_CHANNEL_USERNAME.lstrip('@')}" if PLATFORM == "telegram" else f"https://ble.ir/{INFO_CHANNEL_USERNAME.lstrip('@')}"
    return {"text": "📢 کانال اطلاع‌رسانی", "url": url}

def create_retry_button(callback_data: str, user_id: int = None, button_text: str = "🔄 تلاش مجدد"):
...
def create_pagination_row(callback_prefix: str, current_page: int, total_pages: int, user_id: int = None):
    ...
    buttons, suffix = [], f":u{user_id}" if user_id else ""
    if current_page > 1:
...
def get_confirmation_keyboard(setting_type, new_value, user_id: int = None):
    suffix = f":u{user_id}" if user_id else ""
    return [[{"text": "✅ بله، تغییر کن", "callback_data": f"confirm_{setting_type}:{int(new_value)}{suffix}"}, {"text": "❌ خیر، انصراف", "callback_data": f"back_to_settings{suffix}"}]]

def get_search_choice_keyboard(user_id: int, query_id: str):
    suffix = f":u{user_id}" if user_id else ""
    return [[{"text": "🔍 جستجو در مینی‌اپ", "web_app": "https://3rah.ir/music/ui"}], [{"text": "💬 جستجو در چت", "callback_data": f"search_chat:{query_id}{suffix}"}], [create_close_button(user_id)]]
