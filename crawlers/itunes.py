...
async def lookup_itunes(id: Union[int, str], entity: Optional[str] = None, bypass_cache: bool = False, status_msg: Message = None, status_text: str = None, official: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[Message]]:
    if status_msg and status_text:
        status_msg = await edit_message(status_msg, status_text, show_cancel=True, force_edit=True)
    data = await fetch_itunes("lookup", {"id": id, "entity": entity} if entity else {"id": id}, bypass_cache=bypass_cache, official=official)
