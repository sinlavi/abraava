...
async def get_track(track_id: Union[int, str], status_msg: Message = None) -> Tuple[Optional[Dict[str, Any]], Optional[Message]]:
    if OFFLINE_MODE: return None, status_msg

    if isinstance(track_id, str):
        if track_id.startswith("sp_"):
            track = await music_adapter.get_sp_track(track_id)
            return ({"results": [track]} if track else None), status_msg
...
async def get_or_crawl_artist(artist_id: Union[int, str], status_msg: Optional[Message] = None, force: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[Message]]:
    if OFFLINE_MODE: return None, status_msg

    if isinstance(artist_id, str):
        if artist_id.startswith("sp_"):
            artist = await music_adapter.get_sp_artist(artist_id)
            return ({"results": [artist]} if artist else None), status_msg
...
async def get_or_crawl_collection(collection_id: Union[int, str], status_msg: Optional[Message] = None, force: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[Message]]:
    if OFFLINE_MODE: return None, status_msg

    if isinstance(collection_id, str):
        if collection_id.startswith("sp_"):
            album = await music_adapter.get_sp_album(collection_id)
            return ({"results": [album]} if album else None), status_msg
...
async def get_or_crawl_artist_collections(artist_id: Union[int, str], status_msg: Optional[Message] = None, force: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[Message]]:
    if OFFLINE_MODE: return None, status_msg

    if isinstance(artist_id, str):
        if artist_id.startswith("sp_"):
            albums = await music_adapter.get_sp_artist_albums(artist_id)
            return ({"results": albums} if albums else None), status_msg
        elif artist_id.startswith("it_"):
...
async def get_or_crawl_collection_tracks(collection_id: Union[int, str], status_msg: Optional[Message] = None, force: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[Message]]:
    if OFFLINE_MODE: return None, status_msg

    if isinstance(collection_id, str):
        if collection_id.startswith("sp_"):
            tracks = await music_adapter.get_sp_album_tracks(collection_id)
            return ({"results": tracks, "resultCount": len(tracks)} if tracks else None), status_msg
        elif collection_id.startswith("yt_"):
            tracks = await music_adapter.get_yt_album_tracks(collection_id)
            return ({"results": tracks, "resultCount": len(tracks)} if tracks else None), status_msg
        elif collection_id.startswith("it_"):
            data, status_msg = await lookup_itunes(collection_id[3:], "song", bypass_cache=force, status_msg=status_msg, status_text="⏳ *در حال دریافت آهنگ‌های آلبوم از آیتیونز رسمی...*", official=True)
            if data and data.get("results"):
                data["results"] = data["results"][1:]
                data["resultCount"] = len(data["results"])
            return data, status_msg

    data, status_msg = await lookup_itunes(collection_id, "song", bypass_cache=force, status_msg=status_msg, status_text="⏳ *در حال دریافت آهنگ‌های آلبوم از آیتیونز...*")
    if data and data.get("results"):
