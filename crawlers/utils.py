...
async def get_track(track_id: Union[int, str], status_msg: Message = None) -> Union[Optional[Dict[str, Any]], Tuple[Optional[Dict[str, Any]], Optional[Message]]]:
    if OFFLINE_MODE: return (None, status_msg) if status_msg else None

    if isinstance(track_id, str):
        if track_id.startswith("sp_"):
            track = await music_adapter.get_sp_track(track_id)
            res = {"results": [track]} if track else None
...
async def get_or_crawl_artist(artist_id: Union[int, str], status_msg: Optional[Message] = None, force: bool = False):
    ...
    if isinstance(artist_id, str):
        if artist_id.startswith("sp_"):
            ...
            res = {"results": [artist]} if artist else None
            return (res, status_msg) if status_msg else res
...
async def get_or_crawl_collection(collection_id: Union[int, str], status_msg: Optional[Message] = None, force: bool = False):
    ...
    if isinstance(collection_id, str):
        if collection_id.startswith("sp_"):
            ...
            res = {"results": [album]} if album else None
            return (res, status_msg) if status_msg else res
...
async def get_or_crawl_artist_collections(artist_id: Union[int, str], status_msg: Optional[Message] = None, force: bool = False):
    ...
    if isinstance(artist_id, str):
        if artist_id.startswith("sp_"):
            ...
            res = {"results": albums} if albums else None
            return (res, status_msg) if status_msg else res
        elif artist_id.startswith("it_"):
...
async def get_or_crawl_collection_tracks(collection_id: Union[int, str], status_msg: Optional[Message] = None, force: bool = False):
    ...
    if isinstance(collection_id, str):
        if collection_id.startswith("sp_"):
            ...
            res = {"results": tracks, "resultCount": len(tracks)} if tracks else None
            return (res, status_msg) if status_msg else res
        elif collection_id.startswith("yt_"):
            ...
            res = await lookup_itunes(collection_id[3:], "song", bypass_cache=force, status_msg=status_msg, status_text="⏳ *در حال دریافت آهنگ‌های آلبوم از آیتیونز رسمی...*", official=True)
            data, status_msg = (res[0], res[1]) if isinstance(res, tuple) else (res, status_msg)
            if data and data.get("results"):
                data["results"] = data["results"][1:]
                data["resultCount"] = len(data["results"])
            return (data, status_msg) if status_msg else data

    res = await lookup_itunes(collection_id, "song", bypass_cache=force, status_msg=status_msg, status_text="⏳ *در حال دریافت آهنگ‌های آلبوم از آیتیونز...*")
    data, status_msg = (res[0], res[1]) if isinstance(res, tuple) else (res, status_msg)
    if data and data.get("results"):
