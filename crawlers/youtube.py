...
def _get_ytmusic_instance():
    global YT
    if YT is None:
        auth = AuthService.get_ytmusic_auth()
        try:
            YT = YTMusic(auth=auth) if auth else YTMusic()
...
def _build_search_ydl_opts(method: int, preferred_quality: int) -> dict:
    ...
    # Use OAuth for yt-dlp if available
...
def _sync_search_youtube(t_name: str, a_name: str, collection_name: str, ye: str) -> Optional[str]:
    ytm = _get_ytmusic_instance()
    search_query = f"{t_name} {a_name} {collection_name}".strip()

    try:
        results = ytm.search(search_query, filter="songs", limit=10)
        ...
        for res in results:
            ...
            if ye and res_year and str(ye) == str(res_year):
                score += 0.05
...
def get_artist_image(artist_name):
    ...
    try:
        ...
        if not artist_info: return None
