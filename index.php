    ...
    if (!$hasPlatform) {
        ...
        try {
            ...
            $db->exec("INSERT INTO entityMirrors_new (entityType, entityId, urlType, mirrorUrl, quality, platform, updatedAt)
                       SELECT entityType, entityId, urlType, mirrorUrl, quality, 'bale', updatedAt FROM entityMirrors");
            ...
            $db->exec("ROLLBACK");
            error_log("Migration failed for entityMirrors platform support: " . $e->getMessage());
        }
    }
}
    ...
    if (OFFLINE_FALLBACK_ENABLED && isset($params['id'])) {
        ...
        foreach ($ids as $idStr) {
            ...
            foreach (['artist', 'collection', 'track'] as $type) {
                ...
                if ($cached) {
                    attachMirrors($cached, $type, $id, null, $platform);
                    $results[] = $cached;
                    break;
                }
            }
...
function handleRequest(): void {
    ...
    // Extract platform parameter (default to bale)
    $platform = 'bale';
